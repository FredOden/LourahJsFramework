        ...
        ,$llPalmares: {
          ...
          ,$bCases: {
           ...
           ,$bDeathRateByCase: {
            ...
            ,setEnabled: false
            ,setOnClickListener: {
              onClick: v => doPalmares(v, "DeathRateByCase")
              }
            }
          }
        ,$status: {
...
Activity.setTitle("Covid-19 Analyzer 2.0");
...
function doPalmares(b, kind) {
  try{
    var Color = Lourah.graphics.Color;
    drawPalmares(b.getText().toString()
      ,[
        { name: "France", color: Color.CYAN }
        ,{ name: "Italy", color: Color.argb(255,0,255,0) }
        ...
        ,{ name: "China", color: Color.argb(255,200,124,254)}
        ,{ name: "United_Kingdom", color: Color.argb(255,124,234,172)}
        ,{ name: "Sweden", color: Color.argb(255,124*1.5,34*1,172*1.2)}
...
/*
* to work om custom statistics
* should compute yMin and yMax by country
*
*/
function getSerialCount(country, kind) {
  country.serial = [];
...
function drawPalmares(title, countries, kind) {
  ...
  countries.forEach(country => {
      ...
      //var last = data[data.length - 1].computed;
      var y;
      for(var i = Math.max(data.length - 40,0); i < data.length; i++) {
        switch(kind) {
          case "Deaths":
          case "Cases":
  ...
  .forEach((country, i) => {
      ...
      layer.addLabel(
        country.name + ":" + country.score.toFixed(2)
        + ""
        //"(" + d.computed[kind].total + ")"
  ...
  var r = $screen.$date.getText().toString().split("/");
  layer.addLabel(
    "Each country " + title //+ "(absolute value)"
    + " " + r[2] + "/" + r[1] + "/" + r[0]
