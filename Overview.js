...
Activity.importScript(Lourah.jsFramework.parentDir() + "/Internationalizer.js");
  ...
  var cl;
   try {
	cl = Class.forName(view["class"]);
		...
		//throw view["class"] + "::ZZ::" + e;
		//Activity.reportError(key + "::" + "is a widget");
		widgets[key] = view;
        return view;
	}
	var cons = cl.getConstructor(Packages.android.content.Context);
	var widget = cons.newInstance(Activity.getApplicationContext());
...
var Widget = function(className) {
	...
	this.content = {};
	this.setContent = (name, widget) => {
		this.content[name] = widget;
		}
	}
	
Lourah.android.Overview.LinearLayout = function() {
	return new Widget("android.widget.LinearLayout");
	}
	
Lourah.android.Overview.TextView = function() {
	return new Widget("android.widget.TextView");
	}
	
Lourah.android.Overview.Sugar = function (sugarForm) {
  //console.log("sugarForm::" + JSON.stringify(sugarForm));
  var parse = (o) => {
    try {
      var ov = {};
      for(item in o) {
