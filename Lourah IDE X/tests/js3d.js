...
Lourah.js3d.Renderer = function(width, height) {
	...
	var transform = Lourah.js3d.transform;
	var project = Lourah.js3d.project;

    /**
    zordered 2d points
    **/
	var storePoint2d = function(aw) {
		var i = aw.length;
		while(i--) {
			var w = aw[i];
			if ((w[0] < 0 || w[0] > width )|| (w[1] < 0 || w[1] > height)) continue;
			var wP;
		    var zp = zP[wP = (w[0]|0) + (w[1]|0)*(width|0)];
		    if (zp === undefined || zp[2] <= w[2]) {
			   (zP[wP] = w)[4] = wP;
			   }
			}
		};
	...
	**/
	var storePoint3d = (p, color, rotation, translation, camera) => {
		var pr = transform(p, rotation, translation);
		var p2 = project(pr, camera, [width, height]);
		storePoint2d([[
			p2[0], p2[1],
			pr[2],
			color
	...
	*/
	this.buildLine2d = function(p2d1, p2d2, z1, z2, color) {
        ...
        var d = (2 + (v0 < 0?-v0:v0) + (v1 < 0?-v1:v1));
		var step = 1.0/d;
		var aw = (color)?[]:new Array(d|0);
		v0 *= step;
		v1 *= step;
		vz *= step;
		var w;
    ...
    **/
	    this.line = (p1, p2, color, rotation, translation, camera) => {
		   var pr1 = transform(p1, rotation, translation);
		   var pr2 = transform(p2, rotation, translation);
		   this.buildLine2d(
		...
		**/
		this.polygon =  (ap, color, rotation, translation, camera) => {
			var aap = ap;
			aap.push(ap[0]);
			this.polyLine(aap, color, rotation, translation, camera);
			};
		...
		**/
		this.txel = (p1, p2, p3, color, rotation, translation, camera, spot) => {
			...
			var p2d1 = project(pr1, camera, [width, height])
			...
			var v1, v2, v3;
			var va = this.vector(pr1, pr2)
			    , vb = this.vector(pr2, pr3)
			    , vc = this.vector(pr3, pr1)
			    ;
			
			var vam = this.manhattan(va)
			...
			var la = this.buildLine2d(p2d1, p2d2, pr1[2], pr2[2])
		...
		**/
		this.flush = (imageData) => {
			...
			for(i in zP) {
				  ...
				  zp = zP[i];
				  argb = zp[3];
				  imageData.setPixel(zp[0], zp[1], android.graphics.Color.argb(argb[3], argb[0], argb[1], argb[2]));
				//} else i4 = 0;
				}
			console.log("time::" + ((java.lang.System.currentTimeMillis() - start)/1000));
		    console.log("zP.length::" + count++);
			return imageData;
		};
...
Lourah.js3d.color.negative = ([r,g,b,a]) => (
  [255 - r, 255 - g, 255 - b, a?a:255]
);

Lourah.js3d.color.rgb = (r, g, b) => android.graphics.Color.argb(255, r, g, b);//([r, g, b, 255]);
Lourah.js3d.color.rgba = (r, g, b, a) => android.graphics.Color.argb(a, r, g, b); //([r, g, b, a]);
